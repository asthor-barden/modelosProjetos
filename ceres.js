function createCeres() {
    ...
    const gearboxGeometry = new THREE.BoxGeometry(1.1, 2.3, 1.2); // Mesmas dimensões do "Press Switch"
    const gearboxMaterial = new THREE.MeshPhongMaterial({ 
        color: 0xFFFF00, // Amarelo
        shininess: 30 
    });
    const gearbox = new THREE.Mesh(gearboxGeometry, gearboxMaterial);
    gearbox.position.set(0, 0.8, 0); // No topo do motor (1.5/2 + 0.7/2 + ajuste)
...
const motor3Geometry = new THREE.CylinderGeometry(0.3, 0.35, 0.1, 32);
    ...
    leftShaft.position.set(-0.7, 0.2, 0); // Alinhado ao centro da caixa de redução
    ...
    rightShaft.position.set(0.7, 0.2, 0); // Alinhado ao centro da caixa de redução
